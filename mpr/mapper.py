...
def mapper_pool(G, preimages, device='cpu'):
    ...
    """
    mnode_to_nodes = np.empty((len(preimages), G.number_of_nodes())).to(device)
    current_adj = torch.tensor(
        nx.to_numpy_matrix(G, nodelist=np.arange(G.number_of_nodes()), dtype=np.float32)).to(device)

    # Each preimage has a corresponding colour given by its index.
    for pre_img in preimages:
        # Skip if the preimage is empty
        if not len(pre_img):
            continue

        # Make each connected component a node and assign its color.
        adj_row = torch.zeros(G.number_of_nodes()).to(device)
        adj_row[np.fromiter(pre_img, int, len(pre_img))] = 1.0
        mnode_to_nodes.append(adj_row)

    # Initialise Mapper graph
    mnode_to_nodes = torch.stack(mnode_to_nodes).to(device)
    mnode_to_nodes = mnode_to_nodes / torch.sum(mnode_to_nodes, dim=0)
...
def mpr_pool(x, adj, clusters, overlap, EPS=1e-9):
    ...
    keys = np.array(pr_dict.keys(), dtype=np.int32)
    values = np.array(pr_dict.values(), dtype=np.float32)
    pr[keys] = values
    ...
    St, A = mapper_pool(graph, preimages)
